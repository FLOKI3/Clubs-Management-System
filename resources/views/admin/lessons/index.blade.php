    ...
    <div class="flex flex-col">
        @can('Create lessons')
                <div class="flex justify-end">
                    <a href="{{ route('admin.lessons.create') }}" type="button"
                        class="ml-2 text-white focus:ring-4 focus:outline-none font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center bg-green-600 hover:bg-green-700 focus:ring-green-800">Create Lesson
                    </a>
                </div>
        @endcan
                ...
                <div>
                    <table id="example" class="w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                <th scope="col" class="px-6 py-3 text-start text-xs font-medium text-gray-500 uppercase">Lesson name</th>
                        ...
                        @foreach ($lessons as $lesson)
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                    ...
                                    @can('Edit lessons')
                                    <a href="{{ route('admin.lessons.edit', $lesson->id) }}" type="button" class="text-white focus:ring-4 focus:outline-none font-medium rounded-full text-sm p-2.5 text-center inline-flex items-center me-2 bg-blue-600 hover:bg-blue-700 focus:ring-blue-800">
                                    ...
                                    @endcan
                                    @can('Delete lessons')
                                    <form method="POST" action="{{ route('admin.lessons.destroy', $lesson->id) }}" onsubmit="return confirm('Are you sure?');">
                                        @csrf
                                        @method('DELETE') 
